...
구간 반복을 켜면 타임라인 위의 `IN`·`OUT` 핸들을 드래그하거나 반복 설정 막대에
시작·끝 시간을 직접 입력할 수 있습니다. 재생 헤드의 현재 위치를 시작 또는 끝으로
지정할 수도 있습니다. `Shift`를 누른 채 타임라인을 드래그한 뒤 `선택 범위 사용`을
누르면 선택한 전체 구간이 반복 범위가 됩니다.
